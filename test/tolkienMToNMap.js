...
import {TolkienMToNMap} from "../src/util.js";

describe("TolkienMToNMap", function () {
    beforeEach(function () {
        this.map = new TolkienMToNMap([
            ["x", "z"],
            ["x", "y"],
            ["answer", 42],
            ["number", 42],
    ...
    describe("#clear", function () {
        ...
        it("should fire an event", function () {
            ...
            expect(keys).to.have.members(["x", "answer", "number", "key"]);
            expect(values).to.have.members(["z", "y", 42, "value"]);
        });
    });
    ...
    describe("#xs", function () {
        it("should yield all x-values in the map", function () {
            const r = [...this.map.xs()];
            expect(r).to.have.members(["x", "answer", "number", "key"]);
        });
    });
    ...
    describe("#ys", function () {
        it("should yield all y-values in the map", function () {
            const r = [...this.map.ys()];
            expect(r).to.have.members(["z", "y", 42, "value"]);
        });
    });
